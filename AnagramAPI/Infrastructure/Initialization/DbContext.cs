...
namespace AnagramAPI.Infrastructure.Initialization
{
    internal class DbContext : IServicesRegistration
    {
        ...
        {
            services.AddDbContext<AnagramDbContext>(options => options.UseMySql(configuration.GetConnectionString("InformationDb"), b => b.MigrationsAssembly(typeof(AnagramDbContext).Assembly.FullName)));
